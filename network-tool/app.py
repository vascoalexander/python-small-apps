...
class App(ttk.Window):
    ...
    def __init__(self):
        ...
        self.button_submit = ttk.Button(
            self, text="Submit", command=self.show_network, takefocus=False
        )
    ...
    def show_network(self):
        ...
        try:
            ...
            messagebox.showerror(
                ...
                message=f"Something went wrong. Check IP and Subnetmask. Error: {e}",
            )
    ...
    def validate_ip(self):
        ...
        try:
            ...
            messagebox.showwarning(
                title="Invalid IP", message="Invalid IP-Address. Please enter a valid IP"
            )
            ip = "0.0.0.0"
    ...
    def get_mask(self):
        ...
        if not self.entry_ip.cidr_field.instate(["readonly"]):
            ...
            if subnetmask not in net.cidr_to_subnet.values():
                messagebox.showwarning(
                    title="Invalid Mask", message="Invalid Subnetmask. Please enter a valid mask"
                )
                subnetmask = "0.0.0.0"
    ...
    def update_subnetmask(self, subnetmask):
        ...
        subnetmask entries"""
        if self.entry_sub.octet_list[0].instate(["readonly"]):
            self.entry_sub.toggle_state("normal")
            for index, octet in enumerate(self.entry_sub.octet_list):
                octet.delete(0, tk.END)
                octet.insert(0, subnetmask[index])
...
class Octett(ttk.Entry):
    ...
    def __init__(self, parent, text, state):
        super().__init__(parent, justify="center", width="5", foreground="lightgrey", state=state)
...
class FrameResult(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        image_data = b""
        self.pil_image = Image.open(BytesIO(base64.b64decode(image_data)))
