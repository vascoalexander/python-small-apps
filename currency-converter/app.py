    ...
    try:
        if curr_to != "btc" and curr_to != "eth":
            converted_currency = str(
                f"{(api_response[curr_to] * int(abs(currency_amount.get()))):.2f}"
            )
        else:
            converted_currency = str(
                f"{(api_response[curr_to] * int(abs(currency_amount.get()))):.5f}"
            )

    except Exception as e:
        messagebox.showerror(
            title="Input Error", message=f"Input error: {e}. Enter a valid number."
        )
